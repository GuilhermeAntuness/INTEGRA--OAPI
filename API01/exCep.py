...
def buscar_cep():
    while True:
        try:
            ...
            meu_cep = response.json()

            if 'erro' in meu_cep:
                print('Erro de CEP inválido.')
                continue

            print(f'{meu_cep.get("cep"), meu_cep.get("logradouro"), meu_cep.get("bairro")}')
            confirmar = input('Confirmar endereço [S/N]: ')

            if confirmar.lower() == 's':
                return f'{meu_cep["cep"], meu_cep["logradouro"],  meu_cep["bairro"]}'
...
def editar_contato(lista_tipo_contato):
    ...
    if 0 <= codigo < len(lista_tipo_contato):
        ...
        cep = input("Digite o novo cep (ou deixe em branco para manter o mesmo): ")
        ...
        if cep:
            lista_tipo_contato[codigo]["endereco"] = cep
